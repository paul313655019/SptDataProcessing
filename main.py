...
df = df.groupby(['TRACK_ID', 'FileID']).apply(analysis.calculate_diff_d).reset_index(drop=True)
df.info()
df.head()
...
grouped_df = df.groupby(['FileID', 'TRACK_ID'])['D'].mean().to_frame()
...
fig.show()
# %%
df2 = df.groupby(['FileID', 'TRACK_ID']).apply(analysis.calculate_msd).reset_index(drop=True)

# %%
# Plot MSD vs Lag_T for each FileID and TRACK_ID
fig = px.line()

for (file_id, track_id), group in df2.groupby(['FileID', 'TRACK_ID']):
...
fig.update_layout(
    ...
    paper_bgcolor='rgb(255, 255, 255)',
    plot_bgcolor='rgb(255, 255, 255)',
    title='MSD vs Lag_T for all FileIDs and TRACK_IDs'
)
